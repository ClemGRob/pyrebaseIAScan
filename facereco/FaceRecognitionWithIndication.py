...
class FaceRecognition:
    def __init__(self):
        ...
        self.input_ID = ""
        self.input_password = ""
        self.face_cascade = cv2.CascadeClassifier('/home/clem/pyrebaseIAScan/facereco/haarcascade_frontalface_default.xml')
    ...
    def capture_photos(self, name_client):
        ...
        while self.images_per_person_dict[name_client] < self.images_per_person:
            ...
            for (x, y, w, h) in faces_detected:
                ...
                if key == ord(' '):
# 
                    img_name = os.path.join('dataset', name_client,f"{name_client}{self.images_per_person_dict[name_client] + 1}.jpg")
    ...
    def register_faces(self, name, last_name, apartment_number, is_house_admin=False):
        ...
        for i in range(self.images_per_person):
            img_path = os.path.join('dataset', full_name, f"{full_name}{i + 1}.jpg")
        ...
        upload(self.storage,"face_recognition_model.yml",self.user)
        
        if os.path.exists("face_recognition_model.yml"):
                shutil.rmtree("face_recognition_model.yml")
        if os.path.exists("../dataset/"+full_name):
                shutil.rmtree("../dataset/"+full_name)
    ...
    def delete_person(self, name, last_name, apartment_number):
        ...
        if full_name in self.house_administrator_dict:
            ...
            with open('house_administrator_dict.json', 'w') as file:
                json.dump(self.house_administrator_dict, file)
            upload(self.storage,'house_administrator_dict.json','house_administrator_dict.json',self.user)
        ...
        with open('images_per_person_dict.json', 'w') as file:
            json.dump(self.images_per_person_dict, file)
        upload(self.storage,'images_per_person_dict.json','images_per_person_dict.json',self.user)
    ...
    def recognize_faces(self):
        ...
        download(self.storage,"face_recognition_model.yml","face_recognition_model.yml",self.user)
